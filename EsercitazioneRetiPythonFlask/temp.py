...
def temperatura():
    ...
    for _ in range(20):
        ...
        temps = psutil.sensors_temperatures()  # Ottiene i dati della temperatura

        # Controlla i sensori disponibili
        if temps:
            coretemp = temps.get('coretemp', [])  # Ottiene i dati dal sensore coretemp
            if coretemp:
                temperatures.append(coretemp[0].current)  # Aggiunge la temperatura corrente
            else:
                temperatures.append(None)  # Se non ci sono dati, aggiunge None
        else:
            temperatures.append(None)  # Se non ci sono sensori, aggiunge None

    # Controlla che le liste di utilizzo CPU e temperatura abbiano la stessa lunghezza
    if len(cpu_usages) != len(temperatures):
        raise ValueError("Le liste di utilizzo della CPU e temperatura devono avere la stessa lunghezza.")
    ...
    sns.lineplot(x=range(len(temperatures)), y=temperatures, label='Temperatura')  # Grafico della temperatura
    plt.xlabel('Tempo')  # Etichetta dell'asse x
    plt.ylabel('Valore')  # Etichetta dell'asse y
    plt.title('Utilizzo CPU e Temperatura nel Tempo')  # Titolo del grafico
